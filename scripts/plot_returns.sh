...
mkdir -p "${PERIOD_DIR}/portfolio_data/${NAME}"

python "${DIR}/scripts/gen_portfolio_returns.py" \
       "${PORTFOLIO_DIR}/assets_data" "${PORTFOLIO_DIR}/portfolio_data/${NAME}" \
       "${PORTFOLIO_DIR}/portfolio_data/${NAME}/port.ind" "${PORTFOLIO_DIR}/portfolio_data/${NAME}/port.fit" \
       "${PERIOD_DIR}/assets_data" "${PERIOD_DIR}/portfolio_data/${NAME}" "${START_YEAR}-01-01" "${END_YEAR}-12-31"

mkdir -p "${PERIOD_DIR}/plot_data/${NAME}"

printf "\nGenerating portfolios plot data\n"
python "${DIR}/scripts/gen_plot_data.py" "${PERIOD_DIR}/portfolio_data/${NAME}/balanced_portfolio.ret" "${PERIOD_DIR}/plot_data/${NAME}/balanced_portfolio.dat" Balanceado
printf "Generated best performance portfolio (balanced_portfolio.dat)\n"
python "${DIR}/scripts/gen_plot_data.py" "${PERIOD_DIR}/portfolio_data/${NAME}/risky_portfolio.ret" "${PERIOD_DIR}/plot_data/${NAME}/risky_portfolio.dat" Arriscado
printf "Generated most return portfolio (risky_portfolio.dat)\n"
python "${DIR}/scripts/gen_plot_data.py" "${PERIOD_DIR}/portfolio_data/${NAME}/safe_portfolio.ret" "${PERIOD_DIR}/plot_data/${NAME}/safe_portfolio.dat" Seguro
